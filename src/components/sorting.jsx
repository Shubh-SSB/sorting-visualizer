...
const SortingVisualizer = ({ darkMode }) => {
  ...
  const [algorithm, setAlgorithm] = useState(
    localStorage.getItem("selectedAlgorithm") || "BubbleSort"
  );

  useEffect(() => {
    localStorage.setItem("selectedAlgorithm", algorithm);
  }, [algorithm]);
  const selalgo = localStorage.getItem("selectedAlgorithm") || "BubbleSort";
  ...
  const handleSort = async () => {
    ...
    if (steps.length > 0) {
      ...
      setTimeout(() => {
        ...
        setActiveIndex([]);
        animateSortedBars([...Array(steps[steps.length - 1].array.length).keys()]);
      }, steps.length * 50);
    }
    startAutoPlay(steps);
  };
  ...
  return (
    <div className="flex flex-row">
      <div className="flex flex-col items-center mt-2">
        ...
        <div className="flex mx-4 space-x-2 mt-10 mb-3 rounded-lg items-end h-[300px]">
          {array.map((value, index) => (
            <div
              key={index}
              className={`w-4 transition-all duration-500 ease-in-out ${sorted.includes(index)
                ? "bg-purple-700 rounded-lg"
                : activeIndex.includes(index)
                  ? "bg-red-500 rounded"
                  : darkMode ? "bg-white rounded-lg" : "bg-black rounded-lg"
                }`}
